...
// This program builds postdeploy.
//
...
const ProjectNamespace = "postdeploy.ak7.io"
...
var (
	...
	buildPackages = []string{
		fmt.Sprintf("%s/cmd/postdeploy", ProjectNamespace),
	}

	// a regular expression matching all non-standard go library packages (e.g. github.com/..., postdeploy.ak7.io/... )
	...
	// The current version number (e.g. "2015-01-25-284c030+")
	version = gitVersion()
)
...
// Get the command for cross-compiling the specified package for the desired operating system and architecture (e.g. 2015-01-25-284c030+).
func getCrossCompilationCommand(packageName, os, arch string) (command string, args []string) {
	dockerImageName := "golang:1.4-cross"
	projectPathInDocker := "/go"
...
// Get the build version flag for the go linker (e.g. -X postdeploy.ak7.io/modules/buildinfo 2015-01-25-284c030+).
func getBuildVersionFlag() string {
	return fmt.Sprintf(`--ldflags=-X %s.GitInfo %s`, "postdeploy.ak7.io/modules/buildinfo", version)
}
